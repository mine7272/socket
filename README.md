...
## 목차
- [TCP \& WebSocket 기반 파일 전송 실습 및 성능 분석 (v2)](#tcp--websocket-기반-파일-전송-실습-및-성능-분석-v2)
  - [목차](#목차)
  - [1. 프로젝트 개요](#1-프로젝트-개요)
  ...
  - [6. 성능 비교 결과](#6-성능-비교-결과)
    - [통신 조합별 실행 시간 정리](#통신-조합별-실행-시간-정리)
  - [7. 성능 차이 원인 분석](#7-성능-차이-원인-분석)
  - [8. 에러 사례 및 해결방안](#8-에러-사례-및-해결방안)
    - [1. WebSocket 프레임 디코딩 실패](#1-websocket-프레임-디코딩-실패)
    - [2. Segmentation fault (세그멘테이션 오류)](#2-segmentation-fault-세그멘테이션-오류)
    - [3. WebSocket 핸드셰이크 실패](#3-websocket-핸드셰이크-실패)
    - [4. client\_rawtcp → server\_tcpws 실패 (v1 시점)](#4-client_rawtcp--server_tcpws-실패-v1-시점)
  - [9. 결론 및 의의](#9-결론-및-의의)
...
|------------------|------------------|---------------------------------------------------------------------|
| client_rawtcp.c  | server_tcpws.c   | WebSocket이 아닌 순수 TCP 전송 → 이제 수신 가능                      |
